...
pub fn validate_region(code: &str) -> Result<(), IdError> {
...
pub fn parse_date(s: &str) -> Result<NaiveDate, IdError> {
    // 先尝试无分隔符格式：19900520
...
pub fn random_seq(gender: Gender) -> String {
    let mut rng = rng();
...
pub fn random_date(min: NaiveDate, max: NaiveDate) -> NaiveDate {
    let mut rng = rng();
...
pub fn checksum_char(id17: &str) -> char {
